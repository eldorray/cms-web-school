<x-layouts.app>
    <div class="space-y-6">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Prestasi</h1>
        </div>
        ...
        <form method="POST" action="{{ route('admin.achievements.update', $achievement) }}" enctype="multipart/form-data"
            ...
            <div
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $achievement->title) }}"
                        required
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required
                            ...
                            @foreach ($types as $key => $label)
                                <option value="{{ $key }}"
                                    {{ old('type', $achievement->type) == $key ? 'selected' : '' }}>{{ $label }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="level" id="level" required
                            ...
                            @foreach ($levels as $key => $label)
                                <option value="{{ $key }}"
                                    {{ old('level', $achievement->level) == $key ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="year" id="year"
                            value="{{ old('year', $achievement->year) }}" required min="2000" max="2100"
                            class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="participant_name" id="participant_name"
                            value="{{ old('participant_name', $achievement->participant_name) }}"
                            class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        ...
                        <input type="text" name="participant_class" id="participant_class"
                            value="{{ old('participant_class', $achievement->participant_class) }}"
                            class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">{{ old('description', $achievement->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    @if ($achievement->getFirstMediaUrl('image'))
                        <div class="mb-3"><img src="{{ $achievement->getFirstMediaUrl('image') }}"
                                class="h-32 rounded-lg object-cover"></div>
                    @endif
